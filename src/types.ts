...
export interface LookerChartUtils {
    Utils: {
        openDrillMenu: (options: { links: Link[]; event: Record<string, any> }) => void;
        openUrl: (url: string, event: Record<string, any>) => void;
...
export interface VisualizationDefinition {
    ...
    trigger?: (event: string, config: Record<string, any>[]) => void;
    update?: (
        data: VisData,
        element: HTMLElement,
        config: VisConfig,
